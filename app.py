...
# --- ANTY-COPY GUARD: BLOKADA MYSZKI, KLAWIATURY I SKRÓTÓW DEWELOPERSKICH ---
st.markdown("""
    <script>
    ...
    // 2. Blokada skrótów klawiszowych (Ctrl+C, Ctrl+U, Ctrl+S, F12, Ctrl+Shift+I)
    document.addEventListener('keydown', function(e) {
        ...
        if (e.ctrlKey && e.keyCode == 85) { // Ctrl+U (Podgląd źródła)
            e.preventDefault();
            return false;
        }
        if (e.ctrlKey && e.keyCode == 83) { // Ctrl+S (Zapisanie strony)
            e.preventDefault();
            return false;
        }
        if (e.ctrlKey && e.keyCode == 67) { // Ctrl+C (Kopiowanie)
            e.preventDefault();
            return false;
        }
    });
    ...
    <style>
    /* 3. Blokada zaznaczania tekstu na poziomie CSS */
    ...
    /* Przyciski i elementy aktywne w rzędzie (Wysokość zablokowana na 32px) */
    .stButton>button {
...
# --- ZABEZPIECZENIE STRUMIENIA TEKSTU PRZED XSS ---
def security_clean_text(val):
    if not isinstance(val, str): return val
    return re.sub(r'<[^>]*?>', '', val).strip()

# --- SYSTEM FUNKCJI GLOBALNYCH ---
...
def render_bracket_match_html_clean(match_id):
    ...
    <div class="bracket-match-card">
        ...
        <div class="bracket-row {"bracket-team-winner" if win_h else ""}"><span>{m.get('home', 'TBD')}</span><span class="bracket-score-cell">{sh}</span></div>
        <div class="bracket-row {"bracket-team-winner" if win_a else ""}"><span>{m.get('away', 'TBD')}</span><span class="bracket-score-cell">{sa}</span></div>
    </div>
    """, unsafe_allow_html=True)
...
# --- STRUKTURA INICJALIZACYJNA SYSTEMU ---
now = datetime.now()
...
if st.session_state.logged_in_user is None:
    ...
    with tab3:
        ...
        for g_name in list(GROUPS_DICT.keys()):
            ...
            g_rows = "".join([f"<tr {'style=\"background-color: #16A34A; font-weight: bold; color: #FFFFFF;\"' if idx in [1, 2] else ('style=\"background-color: #EA580C; font-weight: bold; color: #FFFFFF;\"' if idx == 3 else '')}><td><b>{idx}</b></td><td>{r['Reprezentacja']}</td><td><b>{r['Pkt']}</b></td><td>{r['BZ']}</td><td>{r['BS']}</td><td>{r['RB']}</td></tr>" for idx, r in df_g.iterrows()])
        ...
        if third_places_list:
            ...
            for idx, r in df_third.iterrows():
                third_rows += f"<tr {'style=\"background-color: #16A34A; font-weight: bold; color: #FFFFFF;\"' if idx <= 8 else ''}><td><b>{idx}</b></td><td><b>{r['Grupa']}</b></td><td>{r['Reprezentacja']}</td><td><b>{r['Pkt']}</b></td><td>{r['BZ']}</td><td>{r['BS']}</td><td>{r['RB']}</td></tr>"
